...
public class ClientResourceTest {
    ...
    public void testarEndPointIncomeGreaterThanRetornaCorreto() throws Exception{
        ...
        listaClientes.add(new ClientDTO(new Client(4L, "Carolina Maria de Jesus", "10419244771", 7500.0, Instant.parse("1996-12-23T07:00:00Z"), 0)));
        listaClientes.add(new ClientDTO(new Client(8L, "Toni Morrison", "10219344681", 10000.0,Instant.parse("1940-02-23T07:00:00Z"), 0)));
        listaClientes.add(new ClientDTO(new Client(9L, "Sebastião Siveira", "17256987123", 5000.0,Instant.parse("1952-08-31T15:30:00Z"), 1)));

        Page<ClientDTO> page = new PageImpl<>(listaClientes);

        Mockito.when(
            service.findByIncomeGreaterThan(Mockito.any(), Mockito.anyDouble())
        ).thenReturn(page);
        // fim configuração mockito

        // Act
        ResultActions resultado = mockMVC.perform(get("/clients/incomeGreaterThan/").param("income", String.valueOf(maior)).accept(MediaType.APPLICATION_JSON));
    ...
    public void testarEndPointfindByCPFRetornaCorreto() throws Exception{
        ...
        Mockito.when(
            service.findByCpfLike(Mockito.any(), Mockito.anyString())
        ).thenReturn(page);
        // fim configuração mockito

        // Act
        ResultActions resultado = mockMVC.perform(get("/clients/cpf/").param("cpf", String.valueOf(parteCPF)).accept(MediaType.APPLICATION_JSON));
    ...
    public void testarEndPointInsertRetornaCorreto() throws Exception{
        ...
        //arrange
        ClientDTO newCliente1 = new ClientDTO(7L, "Jose Saramago", "10239254871", 5000.0, Instant.parse("1996-12-23T07:00:00Z"), 0);
        ClientDTO newCliente2 = new ClientDTO(4L, "Carolina Maria de Jesus", "10419244771", 7500.0, Instant.parse("1996-12-23T07:00:00Z"), 1);

        ClientDTO savedCliente1 = new ClientDTO(7L, "Jose Saramago", "10239254871", 5000.0, Instant.parse("1996-12-23T07:00:00Z"), 0);
        ClientDTO savedCliente2 = new ClientDTO(4L, "Carolina Maria de Jesus", "10419244771", 7500.0, Instant.parse("1996-12-23T07:00:00Z"), 1);

        Mockito.when(service.insert(Mockito.any(ClientDTO.class))).thenReturn(savedCliente1).thenReturn(savedCliente2);
        //fim configuração mockito
